...
try {
  ...
  if(state === 'Sign Up'){
   const {data} = await axios.post(backendUrl + '/api/auth/register', {name, email, password, verifyQuestion, verifyAnswer})
   ...
   if(data.success){
    ...
    toast.error(data.message)
     ...
     const {data} = await axios.post(backendUrl + '/api/auth/login', { email, password, verifyQuestion, verifyAnswer})
  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      <form onSubmit={onSubitHandler}>
        ...
        <div className='mb-4 flex item-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          <img src={assets.lock_icon} alt="" />
          <input onChange={e => setVerifyQuestion(e.target.value)} 
           value ={verifyQuestion}
           className='bg-transparent outline-none' type="text" placeholder="verify question" required/>
        </div>

        <div className='mb-4 flex item-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
          <img src={assets.lock_icon} alt="" />
          <input onChange={e => setVerifyAnswer(e.target.value)} 
           value ={verifyAnswer}
           className='bg-transparent outline-none' type="password" placeholder="Verify answer" required/>
        </div>
